...
from miette.cfb.constants import (UNALLOCATED, STORAGE_OBJECT, STREAM_OBJECT,
                                  ROOT_STORAGE_OBJECT, RED, BLACK, MAXREGSID,
                                  NOSTREAM, VERSION_3, ENDOFCHAIN)
...
class Entry(object):
    def __init__(self, entry_id, reader, position):
        ...
        if re.search(r'[/\\:!]', self.name, re.UNICODE):
            raise MietteFormatError("The following characters are illegal "
                                    "and MUST NOT be part of the name: '/', "
                                    "'\\', ':', '!'.")

        if self.object_type == UNALLOCATED:
            raise MietteFormatError("Can't create Directory Entry object "
                                    "for unallocated place")

        if self.object_type not in (STORAGE_OBJECT, STREAM_OBJECT,
                                    ROOT_STORAGE_OBJECT):
            raise MietteFormatError("Object Type MUST be 0x00, 0x01, 0x02, "
                                    "or 0x05, depending on the actual type "
                                    "of object.")

        if self.color_flag not in (RED, BLACK):
            raise MietteFormatError("Color Flag MUST be 0x00 (red) or "
                                    "0x01 (black).")

        if MAXREGSID < self.left_sibling_id < NOSTREAM:
            raise MietteFormatError("Maximum regular stream ID is "
                                    "0xFFFFFFFA (MAXREGSID)")
        ...
        if self.object_type == STREAM_OBJECT:
            if int(self.clsid) != 0:
                raise MietteFormatError("In a stream object, CLSID field MUST "
                                        "be set to all zeroes.")
            if self.state_bits != 0:
                raise MietteFormatError("State bits field MUST be set to all "
                                        "zeroes for a stream object.")
        ...
        #        raise MietteFormatError("For a root storage object, Creation "
        #                                "Time MUST be all zeroes, and the "
        #                                "creation time is retrieved or set "
        #                                "on the compound file itself.")
        #    if self.modified_time:
        #        raise MietteFormatError("For a root storage object, Modified "
        #                                "Time MUST be all zeroes, and the "
        #                                "modified time is retrieved or set "
        #                                "on the compound file itself.")

        if self._reader.major_version == VERSION_3 and \
                self.stream_size > 0x80000000:
            raise MietteFormatError("For a version 3 compound file 512-byte "
                                    "sector size, this value of this field "
        ...
        self.sector_size = self._reader.sector_size \
            if not self.is_mini_sector else self._reader.mini_sector_size
        self.sector_shift = self._reader.sector_shift \
            if not self.is_mini_sector else self._reader.mini_sector_shift
    ...
    def read(self, size=None):
        ...
        while len(read_buffer) < size:
            ...
            if to_read >= to_end:
                ...
                sector_position = \
                    (self._sector_number + int(not self.is_mini_sector)) \
                    << self.sector_shift
    ...
    def _get_next_sector(self, current):
        """
            Wrapper to the reader source
            get_next_fat_sector/get_next_mini_fat_sector
            methods. Transparent to all stream sizes.
        """
        return self._reader.get_next_fat_sector(current) \
            if not self.is_mini_sector else \
            self._reader.get_next_mini_fat_sector(current)
    ...
    def _filetime2timestamp(self, filetime):
        ...
        return datetime.utcfromtimestamp(
            (filetime - 116444736000000000L) / 10000000)
